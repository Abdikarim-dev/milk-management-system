...
function DialogModal(props) {
  // console.log();
  const selectedInTheArray = transactionInfo.filter((transact) => {
    return transact.price === props?.transaction?.price;
  });
  const [selectedInfo, setSelectedInfo] = useState(selectedInTheArray[0]);
  ...
  const editTransaction = async (e) => {
    ...
    const transaction = {
      ...
      price: parseInt(selectedInfo.price),
    ...
    try {
      const response = await updateTransaction(transaction,props?.transaction?.id);
      ...
      <Dialog>
        ...
        <DialogContent>
          <Form>
            ...
            <form onSubmit={editTransaction} className="space-y-8">
